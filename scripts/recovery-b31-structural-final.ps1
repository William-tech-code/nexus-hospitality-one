...
Write-Host " V2.3.0-B31 - STRUCTURAL FINAL RECOVERY" -ForegroundColor Yellow
Write-Host "============================================================" -ForegroundColor Cyan
...
$backup=".\backups\V2.3.0-B31-MASTER-$stamp"
...
/*
==============================================================
B31 - STRUCTURAL SMARTPOS REPAIR
==============================================================

Nao depende de acentos ou mojibake.
Somente estruturas comprovadas pelo mapa forense.
...
Capturamos o template literal sem depender do texto interno.
*/

regexOne(
 "./src/SmartPOSV16.jsx",
 "POS_PEOPLE_AVERAGE_B31",
 /peopleCount>0\s+(`[^`\r\n]*brl\(total\/peopleCount\)[^`\r\n]*`)\s*:\s*brl\(total\)/g,
...
Pode ja ter sido corrigido pelo B29.
Por isso nao e obrigatorio aqui.
--------------------------------------------------------------
*/
...
Tambem pode ja ter sido corrigido pelo B29.
--------------------------------------------------------------
*/

regexOne(
 "./src/SmartPOSV16.jsx",
 "POS_PERSON_METHOD_B31",
 /person\.method===method\.id\s+'selected'\s*:\s*''/g,
 "person.method===method.id ? 'selected' : ''",
 false
);
...
regexOne(
 "./src/SmartPOSV16.jsx",
 "POS_PEOPLE_PROGRESS_B31",
 /people\.length\s+\(peoplePaid\/people\.length\)\s*\*\s*100\s*:\s*0/g,
...
Nao depende do texto AGUARDANDO.
Captura o terceiro branch inteiro.
--------------------------------------------------------------
*/

regexOne(
 "./src/SmartPOSV16.jsx",
 "POS_SPLIT_FINAL_BUTTON_B31",
 /\{busy\s+'FINALIZANDO\.\.\.'\s*:\s*allPeoplePaid\s+'FINALIZAR VENDA'\s*:\s*(`[^`\r\n]*`)\s*\}/g,
...
const broken31=[
  ...
  [
    "PERSON_PAID",
    /person\.paid\s+'paid'\s*:/
  ],
  [
    "PERSON_METHOD",
    /person\.method===method\.id\s+'selected'\s*:/
  ],
  [
    "PEOPLE_PROGRESS",
    /people\.length\s+\(peoplePaid\/people\.length\)\s*\*\s*100\s*:/
  ],
  [
    "SPLIT_FINAL",
    /busy\s+'FINALIZANDO\.\.\.'\s*:\s*allPeoplePaid/
  ],
  [
    "SPLIT_SECOND_BRANCH",
    /allPeoplePaid\s+'FINALIZAR VENDA'\s*:/
  ]
];

let brokenCount31=0;

for(const [name,re] of broken31){

  const found=re.test(smart31);

  console.log(
    `B31_${name}_BROKEN=${found ? 'YES' : 'NO'}`
  );
...
console.log(
  "B31_REMAINING_KNOWN_BROKEN="+brokenCount31
);
...
Write-Host " B31 STRUCTURAL FINAL RECOVERY = PASS" -ForegroundColor Green
Write-Host "============================================================" -ForegroundColor Green
