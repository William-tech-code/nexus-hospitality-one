...
Write-Host " V2.3.0-B30 - SMARTPOS MASTER FINAL RECOVERY" -ForegroundColor Yellow
Write-Host "============================================================" -ForegroundColor Cyan
...
$backup=".\backups\V2.3.0-B30-MASTER-$stamp"
...
/*
==============================================================
B30 - SMARTPOS FORENSIC MAP COMPLETE REPAIR
==============================================================

Somente padrões comprovados pelo mapa read-only.
Nenhum CSS será alterado.
...
peopleCount>0
   `${brl(total/peopleCount)} em média`
  : brl(total)
*/

regexOne(
 "./src/SmartPOSV16.jsx",
 "POS_PEOPLE_AVERAGE_B30",
 /peopleCount>0\s+(`\$\{brl\(total\/peopleCount\)\} em média`)\s*:\s*brl\(total\)/g,
 "peopleCount>0 ? $1 : brl(total)"
);
...
/*
3. METODO SELECIONADO POR PESSOA

person.method===method.id
   'selected'
  : ''
*/
...
regexOne(
 "./src/SmartPOSV16.jsx",
 "POS_PERSON_METHOD_B30",
 /person\.method===method\.id\s+'selected'\s*:\s*''/g,
 "person.method===method.id ? 'selected' : ''"
);

/*
4. PROGRESSO DOS PAGAMENTOS

people.length
   (peoplePaid/people.length)*100
  : 0
*/

regexOne(
 "./src/SmartPOSV16.jsx",
 "POS_PEOPLE_PROGRESS_B30",
 /people\.length\s+\(peoplePaid\/people\.length\)\*100\s*:\s*0/g,
 "people.length ? (peoplePaid/people.length)*100 : 0"
);

/*
5. BOTAO FINAL DO SPLIT
...
Existem DUAS ternarias quebradas nessa mesma expressão.
*/

regexOne(
 "./src/SmartPOSV16.jsx",
 "POS_SPLIT_FINAL_BUTTON_B30",
 /\{busy\s+'FINALIZANDO\.\.\.'\s*:\s*allPeoplePaid\s+'FINALIZAR VENDA'\s*:\s*(`AGUARDANDO \$\{people\.length-peoplePaid\} PAGAMENTO\(S\)`)\s*\}/g,
 "{busy ? 'FINALIZANDO...' : allPeoplePaid ? 'FINALIZAR VENDA' : $1}"
);

/*
==============================================================
VALIDACAO LOCAL DO MAPA
==============================================================
*/
...
const forbidden30=[
  {
    name:"PEOPLE_AVERAGE_STILL_BROKEN",
    re:/peopleCount>0\s+`\$\{brl\(total\/peopleCount\)\} em média`\s*:/
  },
  {
    name:"PERSON_PAID_STILL_BROKEN",
    re:/person\.paid\s+'paid'\s*:/
  },
  {
    name:"PERSON_METHOD_STILL_BROKEN",
    re:/person\.method===method\.id\s+'selected'\s*:/
  },
  {
    name:"PEOPLE_PROGRESS_STILL_BROKEN",
    re:/people\.length\s+\(peoplePaid\/people\.length\)\*100\s*:/
  },
  {
    name:"SPLIT_BUSY_STILL_BROKEN",
    re:/busy\s+'FINALIZANDO\.\.\.'\s*:\s*allPeoplePaid/
  }
];

let forbiddenHits=0;

for(const item of forbidden30){

  const hit=item.re.test(smart30);

  console.log(
    `${item.name}=${hit ? 'FAIL' : 'PASS'}`
  );
...
if(forbiddenHits){
  console.error(
    "B30_FORENSIC_PATTERN_REMAINS="+forbiddenHits
  );
  process.exit(89);
}

console.log("B30_FORENSIC_MAP_PATCHED=PASS");
...
Write-Host " B30 MASTER FINAL RECOVERY = PASS" -ForegroundColor Green
Write-Host "============================================================" -ForegroundColor Green
