...
const brl=v=>Number(v||0).toLocaleString('pt-BR',{style:'currency',currency:'BRL'});const num=v=>Number(v||0).toLocaleString('pt-BR',{maximumFractionDigits:1});
export function PublicCommerceV12(){const[data,setData]=useState(null),[cart,setCart]=useState([]),[step,setStep]=useState('menu'),[form,setForm]=useState({customer_name:'',phone:'',email:'',cpf_cnpj:'',order_type:'PICKUP',address:'',notes:''}),[order,setOrder]=useState(null),[busy,setBusy]=useState(false),[error,setError]=useState('');useEffect(()=>{api.publicStore().then(setData).catch(e=>setError(e.message))},[]);const total=cart.reduce((s,x)=>s+x.qty*Number(x.price),0);function add(p){setCart(c=>{const x=c.find(i=>i.id===p.id);return x?c.map(i=>i.id===p.id?{...i,qty:i.qty+1}:i):[...c,{...p,qty:1}]})}function qty(id,d){setCart(c=>c.map(x=>x.id===id?{...x,qty:Math.max(0,x.qty+d)}:x).filter(x=>x.qty))}async function finish(){setBusy(true);setError('');try{const r=await api.publicCreateOrder({...form,items:cart.map(x=>({product_id:x.id,qty:x.qty}))});setOrder(r);setStep('payment')}catch(e){setError(e.message)}finally{setBusy(false)}}if(error&&!data)return <PublicState title="Loja temporariamente indisponível" text={error}/>;if(!data)return <PublicState title="Preparando sua experiência" text="Carregando cardápio e disponibilidade..."/>;if(step==='payment'&&order)return <PaymentView order={order} title={`Pedido ${order.public_code}`} after="Assim que o Asaas confirmar o pagamento, o pedido entra automaticamente na operação."/>;return <div className="commerce-v12"><header><div><span>NEXUS COMMERCE</span><h1>{data.business}</h1><p>Peça, pague e acompanhe. Retirada ou entrega sem precisar ligar.</p></div><button onClick={()=>setStep(step==='cart'?'menu':'cart')}>Sacola • {cart.reduce((s,x)=>s+x.qty,0)} • {brl(total)}</button></header>{step==='menu'?<main><div className="commerce-grid">{data.products.map(p=><article key={p.id} className={!p.available?'disabled':''}>{p.image_url?<img src={p.image_url}/>:<div className="commerce-placeholder">{p.name?.[0]}</div>}<small>{p.category}</small><h3>{p.name}</h3><p>{p.description||'Preparado para você.'}</p><footer><b>{brl(p.price)}</b><button disabled={!p.available} onClick={()=>add(p)}>{p.available?'Adicionar':'Indisponível'}</button></footer></article>)}</div></main>:<main className="checkout-v12"><section><h2>Seu pedido</h2>{cart.map(x=><div className="checkout-item" key={x.id}><div><b>{x.name}</b><small>{brl(x.price)} cada</small></div><div><button onClick={()=>qty(x.id,-1)}>−</button><span>{x.qty}</span><button onClick={()=>qty(x.id,1)}>+</button></div><strong>{brl(x.qty*x.price)}</strong></div>)}<div className="checkout-total"><span>Total dos itens</span><b>{brl(total)}</b></div></section><section><h2>Como receber?</h2><div className="choice-row"><button className={form.order_type==='PICKUP'?'active':''} onClick={()=>setForm({...form,order_type:'PICKUP'})}>Retirar no local</button><button className={form.order_type==='DELIVERY'?'active':''} onClick={()=>setForm({...form,order_type:'DELIVERY'})}>Receber em casa</button></div><input placeholder="Nome completo" value={form.customer_name} onChange={e=>setForm({...form,customer_name:e.target.value})}/><input placeholder="WhatsApp / telefone" value={form.phone} onChange={e=>setForm({...form,phone:e.target.value})}/><div className="two-fields"><input placeholder="E-mail" value={form.email} onChange={e=>setForm({...form,email:e.target.value})}/><input placeholder="CPF/CNPJ (pagamento)" value={form.cpf_cnpj} onChange={e=>setForm({...form,cpf_cnpj:e.target.value})}/></div>{form.order_type==='DELIVERY'&&<input placeholder="Endereço completo" value={form.address} onChange={e=>setForm({...form,address:e.target.value})}/>}<textarea placeholder="Observações do pedido" value={form.notes} onChange={e=>setForm({...form,notes:e.target.value})}/>{error&&<div className="public-error">{error}</div>}<button className="checkout-pay" disabled={busy||!cart.length||!form.customer_name||!form.phone} onClick={finish}>{busy?'GERANDO PAGAMENTO...':`PAGAR ${brl(total)} COM ASAAS →`}</button></section></main>}</div>}
export function PublicEventsV12({eventId}){
 const[list,setList]=useState([]),[event,setEvent]=useState(null),[buy,setBuy]=useState(null),[form,setForm]=useState({name:'',phone:'',email:'',document:'',quantity:1}),[order,setOrder]=useState(null),[error,setError]=useState(''),[busy,setBusy]=useState(false);
 useEffect(()=>{setError('');if(eventId)api.publicEvent(eventId).then(e=>{setEvent(e);const first=(e.lots||[]).find(l=>l.available);if(first)setBuy(first)}).catch(e=>setError(e.message));else api.publicEvents().then(setList).catch(e=>setError(e.message))},[eventId]);
 if(order)return <PaymentView order={order} title={`Compra ${order.order_code}`} after="Pagamento confirmado = ingresso liberado. Seu QR Code será emitido automaticamente."/>;
 if(error&&!event)return <PublicState title="Eventos" text={error}/>;
 if(!eventId)return <div className="ticket-public-list"><header><div><span>NEXUS TICKETS • VENDA OFICIAL</span><h1>Viva a experiência.<br/><em>Garanta seu lugar.</em></h1><p>Ingressos digitais, pagamento seguro e acesso por QR Code.</p></div></header><main>{list.length?list.map(e=><a href={`/eventos/${e.id}`} key={e.id}><div className="tpl-cover">{e.image_url?<img src={e.image_url} alt=""/>:<div className="tpl-placeholder">N</div>}<span>VENDA OFICIAL</span></div><section><small>{e.starts_at?new Date(e.starts_at).toLocaleString('pt-BR',{dateStyle:'full',timeStyle:'short'}):'Data a confirmar'}</small><h2>{e.title}</h2><p>{e.artist_name||e.event_type} · {e.venue_name||'Local a confirmar'}</p><b>COMPRAR INGRESSO <i>→</i></b></section></a>):<div className="tpl-empty">Nenhum evento disponível para venda neste momento.</div>}</main><footer>NEXUS HOSPITALITY ONE · TICKETING EXPERIENCE</footer></div>;
 if(!event)return <PublicState title="Preparando seu ingresso" text="Carregando lotes e disponibilidade..."/>;
 const lots=event.lots||[],available=lots.filter(l=>l.available),qty=Math.max(1,Math.min(10,Number(form.quantity)||1)),total=Number(buy?.price||0)*qty;
 async function pay(){if(!buy){setError('Escolha um lote para continuar.');return}if(!form.name.trim()||!form.phone.trim()||!form.email.trim()){setError('Preencha nome, WhatsApp e e-mail para continuar.');return}setBusy(true);setError('');try{const r=await api.publicBuyTicket(event.id,{...form,lot_id:buy.id,quantity:qty});setOrder(r)}catch(e){setError(e.message)}finally{setBusy(false)}}
 return <div className="ticket-checkout-premium">
  <header className="tcp-hero">{event.image_url?<img src={event.image_url} alt=""/>:<div className="tcp-fallback"/>}<div className="tcp-shade"/><div className="tcp-top"><a href="/eventos">← TODOS OS EVENTOS</a><span>COMPRA SEGURA · NEXUS TICKETS</span></div><div className="tcp-hero-content"><div className="tcp-badge">● VENDA OFICIAL</div><h1>{event.title}</h1><p>{event.description||`${event.artist_name||event.event_type}${event.venue_name?` · ${event.venue_name}`:''}`}</p><div className="tcp-meta"><span>◷ {event.starts_at?new Date(event.starts_at).toLocaleString('pt-BR',{dateStyle:'long',timeStyle:'short'}):'Data a confirmar'}</span><span>⌖ {event.venue_name||'Local a confirmar'}</span></div><Countdown to={event.sales_ends_at||event.starts_at}/></div></header>
  <main className="tcp-main"><section className="tcp-selection"><span className="tcp-eyebrow">ESCOLHA SEU INGRESSO</span><h2>Qual experiência você quer viver?</h2><p className="tcp-sub">Selecione um lote disponível. O ingresso digital será liberado após a confirmação do pagamento.</p><div className="tcp-lots">{lots.map(l=><button type="button" key={l.id} disabled={!l.available} className={buy?.id===l.id?'selected':''} onClick={()=>{setBuy(l);setError('')}}><span className="tcp-radio">{buy?.id===l.id?'✓':''}</span><div><b>{l.name}</b><small>{l.available?(l.unlimited?'Disponível para compra':l.state==='LAST_UNITS'?`Últimas ${l.remaining} unidade(s)`:`${l.remaining} ingresso(s) disponível(is)`):(l.label||'INDISPONÍVEL')}</small></div><strong>{brl(l.price)}</strong></button>)}</div>{!available.length&&<div className="tcp-soldout">NENHUM LOTE DISPONÍVEL PARA COMPRA AGORA</div>}<div className="tcp-trust"><div><b>QR</b><span>Ingresso digital<small>acesso rápido na portaria</small></span></div><div><b>✓</b><span>Compra protegida<small>confirmação automática</small></span></div><div><b>⚡</b><span>Liberação automática<small>após o pagamento</small></span></div></div></section>
  <aside className="tcp-checkout"><div className="tcp-card"><span className="tcp-eyebrow">FINALIZAR COMPRA</span><h2>Seu ingresso</h2>{buy?<><div className="tcp-summary"><div><span>{buy.name}</span><b>{brl(buy.price)}</b></div><label>QUANTIDADE<div className="tcp-qty"><button type="button" onClick={()=>setForm({...form,quantity:Math.max(1,qty-1)})}>−</button><strong>{qty}</strong><button type="button" onClick={()=>setForm({...form,quantity:Math.min(10,qty+1)})}>+</button></div></label></div><div className="tcp-form"><label>NOME COMPLETO<input autoComplete="name" placeholder="Quem vai receber o ingresso?" value={form.name} onChange={e=>setForm({...form,name:e.target.value})}/></label><label>WHATSAPP<input autoComplete="tel" placeholder="(12) 99999-9999" value={form.phone} onChange={e=>setForm({...form,phone:e.target.value})}/></label><label>E-MAIL<input type="email" autoComplete="email" placeholder="voce@email.com" value={form.email} onChange={e=>setForm({...form,email:e.target.value})}/></label><label>CPF / CNPJ <small>opcional nesta etapa</small><input placeholder="Documento para pagamento" value={form.document} onChange={e=>setForm({...form,document:e.target.value})}/></label></div>{error&&<div className="tcp-error">{error}</div>}<div className="tcp-total"><span>TOTAL</span><strong>{brl(total)}</strong></div><button type="button" className="tcp-buy" disabled={busy||!buy} onClick={pay}>{busy?'PREPARANDO PAGAMENTO...':`COMPRAR AGORA · ${brl(total)}`}<small>PIX · CARTÃO · FORMAS DISPONÍVEIS NO ASAAS</small></button><p className="tcp-safe">🔒 Pagamento processado com segurança. O NEXUS não armazena os dados do seu cartão.</p></>:<div className="tcp-choose">← Escolha um lote para iniciar sua compra.</div>}</div></aside></main>
  <div className="tcp-mobile-buy">{buy&&<><div><small>{buy.name} · {qty}x</small><b>{brl(total)}</b></div><button onClick={()=>document.querySelector('.tcp-checkout')?.scrollIntoView({behavior:'smooth'})}>COMPRAR AGORA</button></>}</div>
  <footer className="tcp-footer"><b>NEXUS TICKETS</b><span>Ingresso digital · QR Code único · Validação na portaria</span></footer>
 </div>}
